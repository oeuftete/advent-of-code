...
from adventofcode.year2018.day8.solution import get_root_node_value, sum_all_metadata
...
@pytest.mark.parametrize("tree_data,metadata_sum", [(TEST_TREE_DATA, 138),])
def test_sum_all_metadata(tree_data, metadata_sum):
    assert sum_all_metadata(tree_data) == metadata_sum


@pytest.mark.parametrize("tree_data,root_node_value", [(TEST_TREE_DATA, 66),])
