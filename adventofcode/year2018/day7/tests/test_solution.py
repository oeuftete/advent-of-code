...
@pytest.mark.parametrize("steps,ordered_step_string", [(TEST_STEPS, "CABDFE"),])
def test_get_ordered_step_string(steps, ordered_step_string):
    assert get_ordered_step_string(steps) == ordered_step_string


@pytest.mark.parametrize("c,offset,cost", [("A", 0, 1), ("A", 60, 61), ("Z", 0, 85),])
def test_step_cost(c, offset, cost):
    return step_cost(c, offset) == cost


@pytest.mark.parametrize("steps,workers,offset,seconds", [(TEST_STEPS, 2, 0, 15),])
