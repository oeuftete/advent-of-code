...
def parse_step(step):
    STEP_FORMAT = re.compile(r"Step (\w) must be finished before step (\w) can begin.")
...
def find_and_pop_next_nodes(g, node, pending_nodes, all_nodes):
    ...
    for next_node in sorted(pending_nodes):
        ...
        if next_node in all_nodes:
            ...
            all_next_node_preds = [
                n for n in list(nx.bfs_tree(g, next_node, True)) if n != next_node
            ]
            preds = list(filter(lambda n: n in pending_nodes, all_next_node_preds))
            if preds:
                logging.debug(
            ...
            logging.debug(
                "{} predecessors ({}) were irrelevant to "
            ...
            recursed = find_and_pop_next_nodes(g, next_node, pending_nodes, all_nodes)
            if recursed:
...
def build_workload_graph(steps, offset):
    ...
    def add_workload_node(c):
        if c not in g:
            g.add_node(c, cost=step_cost(c, offset), completion=math.inf, worker=None)
...
def get_build_time(steps, workers, offset):
    ...
    for i in itertools.count():
        ...
        completed = [n for n, completion in completions.items() if completion == i]
